<script>
  const features = [
    ...
    { icon: '🚨', title: 'Instant Alerts',           desc: 'Get notified the second something breaks — email on all plans, Slack on Pro and Team. Error message included.' },
    { icon: '⚡', title: 'Zero Performance Impact',  desc: 'The SDK sends logs asynchronously in the background. Your script never slows down or blocks on network calls.' },
    { icon: '🐍', title: 'Python & JavaScript',      desc: 'Context manager for Python, auto-monitoring on import for Node.js. Works with cron, GitHub Actions, Railway, anything.' },
    { icon: '📊', title: 'Pass Rate Tracking',       desc: 'See trends over time. Know which scripts are flaky before they cause a real problem. 3 consecutive failures trigger escalated alerts.' },
  ]
</script>
...
<div class="reveal relative z-10 w-full">
  <div class="w-full max-w-[1100px] mx-auto px-6 py-[60px] lg:py-[88px]">
    ...
    <div class="text-center mb-14">
      ...
      <p class="section-subtext">Built for developers who use AI tools to write automation scripts — not for enterprise DevOps teams.</p>
    </div>
